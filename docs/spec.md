...
4. User picks one of four buttons: **Budding**, **Partial bloom**, **Full bloom**, **Fading**. Done. Pin lives on the map.
...
Columns: `id` (uuid, **UUIDv7**), `tree_id` (uuid, fk), `bloom_state` (enum: `budding`, `partial`, `full`, `fading`), `photo_r2_key` (text, nullable), `observed_at` (timestamptz), `reported_by_device` (uuid), `hidden_at` (timestamptz, nullable). A tree's current state is the most recent non-hidden observation. Historical observations are preserved forever — they are the archive.
...
- Once the dataset starts being cited externally, cryptographic provenance has been evaluated — not C2PA in V1 or V2, where the threat model does not justify the complexity, but at this phase, where the archive's citability matters. The question by then is whether tools like TrueCapture or the broader C2PA ecosystem have matured into genuine on-device signing (private key never leaves the phone), not server-side signing that only proves "the server received this bytestream." If yes, evaluate for archive-strengthening purposes only. If no, EXIF plus the moderation queue remain sufficient.
...
- The domain redirects to the archive. The code remains public under Apache 2.0, as it has been from the beginning.
